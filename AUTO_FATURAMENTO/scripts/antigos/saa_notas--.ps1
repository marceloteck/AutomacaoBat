...
try {
  ...
  foreach($p in $producers){
    ...
    Write-Host "TIPO=$($p.tipo)  INSTRUCAO=$($p.instrucao)  NOTAS=$($p.notas.Count)"
    Write-Host "--------------------------------------------------"
    Write-Host ""
  ...
  foreach($ch in $p.notas){
    ...
    # 2) RIGHT e ENTER (novo passo)
    Press-Key("{RIGHT}")
    SleepMs $DELAY_AFTER_RIGHT

    Press-Key("{ENTER}")
    SleepMs $DELAY_AFTER_ENTER2
    ...
    Press-Key($code)
    SleepMs $DELAY_AFTER_TIPO

    Press-Key("{ENTER}")
    SleepMs $DELAY_AFTER_ENTER

    Press-Key("{F4}")
    SleepMs $DELAY_AFTER_F4
